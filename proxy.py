...
def extract_stream_flag(body_bytes):
    """Detect and remove the "stream" key from a /v1/chat/completions request body.

    Returns (body_bytes, is_streaming).

    When "stream" is True: returns the body with the key removed and is_streaming=True.
    In all other cases (absent, False, or invalid JSON): returns the original body
    unchanged and is_streaming=False.
    """
    try:
        data = json.loads(body_bytes.decode('utf-8'))
    except Exception:
...
def response_to_sse_chunks(resp_bytes):
    """Convert a full OpenAI-format JSON response into SSE bytes.

    Emits two data: events followed by data: [DONE]:
      - Content response: first chunk has delta.role + delta.content with
        finish_reason null; second chunk has empty delta with finish_reason "stop".
      - Tool-calls response: first chunk has delta.role + delta.tool_calls with
        finish_reason null; second chunk has empty delta with finish_reason "tool_calls".

    Falls back to wrapping the raw decoded text in delta.content if the response
    ...
    chunk_id = resp.get('id') or 'chatcmpl-stream'
    ...
    if tool_calls:
        first_chunk = dict(base, choices=[{
            'index': 0,
            'delta': {'role': 'assistant', 'tool_calls': tool_calls},
            'finish_reason': None,
        }])
        second_chunk = dict(base, choices=[{
            'index': 0,
            'delta': {},
            'finish_reason': 'tool_calls',
        }])
    ...
    return (b'data: ' + json.dumps(first_chunk).encode('utf-8') + b'\n\n'
            + b'data: ' + json.dumps(second_chunk).encode('utf-8') + b'\n\n'
            + b'data: [DONE]\n\n')
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def _forward(self):
        ...
        if body and self.headers.get('Content-Type', '').startswith('application/json'):
            ...
            body, is_streaming = extract_stream_flag(body)
            if is_streaming:
                sys.stderr.write('[proxy] streaming request detected on /v1/chat/completions\n')
        ...
        try:
            ...
            if is_streaming:
                self._send_streaming(response_to_sse_chunks(resp_body))
            else:
    ...
    def _send_streaming(self, sse_bytes):
