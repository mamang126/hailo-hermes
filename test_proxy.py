...
class TestExtractStreamFlag(unittest.TestCase):

    def test_stream_true_removed_and_flag_set(self):
        body = _j({'model': 'm', 'stream': True})
        out_bytes, is_streaming = proxy.extract_stream_flag(body)
        self.assertTrue(is_streaming)
        out = _d(out_bytes)
        self.assertNotIn('stream', out)
        self.assertEqual(out['model'], 'm')

    def test_stream_false_unchanged_flag_not_set(self):
        body = _j({'model': 'm', 'stream': False})
        out_bytes, is_streaming = proxy.extract_stream_flag(body)
        self.assertFalse(is_streaming)
        self.assertEqual(out_bytes, body)

    def test_stream_absent_unchanged_flag_not_set(self):
        body = _j({'model': 'm'})
        out_bytes, is_streaming = proxy.extract_stream_flag(body)
        self.assertFalse(is_streaming)
        self.assertEqual(out_bytes, body)

    def test_invalid_json_returns_original_no_flag(self):
        bad = b'not json'
        out_bytes, is_streaming = proxy.extract_stream_flag(bad)
        self.assertFalse(is_streaming)
        self.assertEqual(out_bytes, bad)

    def test_other_keys_preserved(self):
        body = _j({'model': 'm', 'stream': True, 'temperature': 0.5, 'messages': []})
        out_bytes, is_streaming = proxy.extract_stream_flag(body)
        self.assertTrue(is_streaming)
        out = _d(out_bytes)
        self.assertNotIn('stream', out)
        self.assertEqual(out['model'], 'm')
        self.assertAlmostEqual(out['temperature'], 0.5)
...
class TestResponseToSseChunks(unittest.TestCase):

    def _content_response(self, content='Hello!', chunk_id='cid-1', model='m', created=1000):
        return _j({
            'id': chunk_id, 'model': model, 'created': created,
            'choices': [{'message': {'role': 'assistant', 'content': content},
                         'finish_reason': 'stop'}]
        })

    def _tool_calls_response(self):
        return _j({
            'id': 'cid-2', 'model': 'm', 'created': 1000,
            'choices': [{'message': {
                'role': 'assistant',
                'content': None,
                'tool_calls': [{'id': 'call_0', 'type': 'function',
                                'function': {'name': 'execute_services', 'arguments': '{}'}}]
            }, 'finish_reason': 'tool_calls'}]
        })

    def test_output_starts_with_data_prefix(self):
        out = proxy.response_to_sse_chunks(self._content_response())
        self.assertTrue(out.startswith(b'data: '))

    def test_output_ends_with_done_terminator(self):
        out = proxy.response_to_sse_chunks(self._content_response())
        self.assertTrue(out.endswith(b'data: [DONE]\n\n'))

    def test_content_response_finish_reason_stop(self):
        out = proxy.response_to_sse_chunks(self._content_response())
        text = out.decode('utf-8')
        # Second chunk should have finish_reason: stop
        chunks = [line[6:] for line in text.split('\n') if line.startswith('data: ') and line != 'data: [DONE]']
        last_chunk = json.loads(chunks[-1])
        self.assertEqual(last_chunk['choices'][0]['finish_reason'], 'stop')

    def test_content_in_first_chunk_delta(self):
        out = proxy.response_to_sse_chunks(self._content_response(content='Hi!'))
        text = out.decode('utf-8')
        chunks = [line[6:] for line in text.split('\n') if line.startswith('data: ') and line != 'data: [DONE]']
        first_chunk = json.loads(chunks[0])
        delta = first_chunk['choices'][0]['delta']
        self.assertEqual(delta.get('content'), 'Hi!')
        self.assertEqual(delta.get('role'), 'assistant')

    def test_tool_calls_finish_reason_is_tool_calls(self):
        out = proxy.response_to_sse_chunks(self._tool_calls_response())
        text = out.decode('utf-8')
        chunks = [line[6:] for line in text.split('\n') if line.startswith('data: ') and line != 'data: [DONE]']
        last_chunk = json.loads(chunks[-1])
        self.assertEqual(last_chunk['choices'][0]['finish_reason'], 'tool_calls')

    def test_tool_calls_in_first_chunk_delta(self):
        out = proxy.response_to_sse_chunks(self._tool_calls_response())
        text = out.decode('utf-8')
        chunks = [line[6:] for line in text.split('\n') if line.startswith('data: ') and line != 'data: [DONE]']
        first_chunk = json.loads(chunks[0])
        ...
        self.assertIsNone(first_chunk['choices'][0]['finish_reason'])

    def test_id_model_created_forwarded(self):
        out = proxy.response_to_sse_chunks(self._content_response(chunk_id='abc', model='my-model', created=9999))
        text = out.decode('utf-8')
        chunks = [line[6:] for line in text.split('\n') if line.startswith('data: ') and line != 'data: [DONE]']
        first_chunk = json.loads(chunks[0])
        self.assertEqual(first_chunk['id'], 'abc')
        self.assertEqual(first_chunk['model'], 'my-model')
        self.assertEqual(first_chunk['created'], 9999)

    def test_missing_id_model_created_use_defaults(self):
        resp = _j({'choices': [{'message': {'role': 'assistant', 'content': 'hi'},
                                'finish_reason': 'stop'}]})
        out = proxy.response_to_sse_chunks(resp)
        text = out.decode('utf-8')
        chunks = [line[6:] for line in text.split('\n') if line.startswith('data: ') and line != 'data: [DONE]']
        first_chunk = json.loads(chunks[0])
        self.assertEqual(first_chunk['id'], 'chatcmpl-stream')
        self.assertEqual(first_chunk['model'], '')
        self.assertIsInstance(first_chunk['created'], int)

    def test_invalid_json_falls_back_to_raw_content_in_delta(self):
        out = proxy.response_to_sse_chunks(b'not json at all')
        self.assertTrue(out.startswith(b'data: '))
        self.assertTrue(out.endswith(b'data: [DONE]\n\n'))
        text = out.decode('utf-8')
        chunks = [line[6:] for line in text.split('\n') if line.startswith('data: ') and line != 'data: [DONE]']
        chunk = json.loads(chunks[0])
        self.assertIn('content', chunk['choices'][0]['delta'])
        self.assertIn('not json', chunk['choices'][0]['delta']['content'])
...
class TestSendStreaming(unittest.TestCase):
    ...
    def _make_handler(self):
        ...
        handler.send_response = lambda code: response_code.__setitem__(0, code)
        handler.send_header = lambda k, v: sent_headers.__setitem__(k.lower(), v)
        handler.end_headers = lambda: None

        return handler, output, response_code, sent_headers

    def test_sends_200_status(self):
        handler, output, response_code, sent_headers = self._make_handler()
        handler._send_streaming(b'data: test\n\ndata: [DONE]\n\n')
        self.assertEqual(response_code[0], 200)

    def test_content_type_is_text_event_stream(self):
        handler, output, response_code, sent_headers = self._make_handler()
        handler._send_streaming(b'data: test\n\ndata: [DONE]\n\n')
        self.assertEqual(sent_headers.get('content-type'), 'text/event-stream')
    ...
    def test_sse_bytes_written_to_socket(self):
        handler, output, response_code, sent_headers = self._make_handler()
        sse = b'data: test\n\ndata: [DONE]\n\n'
        handler._send_streaming(sse)
        self.assertEqual(output.getvalue(), sse)
